...
def hmm_training(
    training_data: list,
    training_data_lengths: list,
    n_states: cython.int = 3,
    random_seed: cython.int = 12345,
    covar: str = "full",
    ...
    if hmm_type == "gaussian":
        hmm_model = GaussianHMM(
            n_components=n_states, covariance_type=covar, random_state=rs, verbose=False
        )
    if hmm_type == "poisson":
...
def hmm_model_save(
    model_file: str,
    hmm_model,
    hmm_binsize: cython.int,
    i_open_region: cython.int,
    i_nucleosomal_region: cython.int,
    i_background_region: cython.int,
    hmm_type: str,
):
    if hmm_type == "gaussian":
...
def hmm_model_init(model_file: str) -> list:
    with open(model_file) as f:
        ...
        hmm_type = m.get(
            "hmm_type", "gaussian"
        )  # if no model_type written, assume older file and use gaussian
        if hmm_type == "gaussian":
            hmm_model = GaussianHMM(
                n_components=3, covariance_type=m["covariance_type"]
            )
