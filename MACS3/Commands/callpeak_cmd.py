...
def run(args):
    ...
    if options.keepduplicates != "all":
        if options.keepduplicates == "auto":
            info(
                "#1 calculate max duplicate %ss in single position based on binomial distribution...",
                tag,
            )
        ...
        if options.PE_MODE:
            info(
                "#1 filter out redundant fragments by allowing at most %d identical fragment(s)",
                treatment_max_dup_tags,
            )
        else:
            info(
                "#1 filter out redundant tags at the same location and the same strand by allowing at most %d tag(s)",
                treatment_max_dup_tags,
            )
    ...
    if control is not None:
        ...
        if options.keepduplicates != "all":
            if options.keepduplicates == "auto":
                info(
                    "#1  for control, calculate max duplicate %ss in single position based on binomial distribution...",
                    tag,
                )
    ...
    if options.nomodel:
        ...
        if options.shift > 0:
            info(
                "#2 Sequencing ends will be shifted towards 3' by %d bp(s)"
                % options.shift
            )
        elif options.shift < 0:
            info(
                "#2 Sequencing ends will be shifted towards 5' by %d bp(s)"
                % (options.shift * -1)
            )
        ...
        try:
            ...
            info(
                "#2 alternative fragment length(s) may be %s bps"
    ...
    if control and options.PE_MODE:
        c1 = (
            c1 * 2
        )  # in PE_MODE, PE data has to be doubled since both ends will be counted for calculating background noise.
    ...
    if options.shift > 0:
        ofhd_xls.write(
            "# Sequencing ends will be shifted towards 3' by %d bp(s)\n"
            % (options.shift)
        )
    elif options.shift < 0:
        ofhd_xls.write(
            "# Sequencing ends will be shifted towards 5' by %d bp(s)\n"
            % (options.shift * -1)
        )
    ...
    if not options.broad:
        ...
        peakdetect.peaks.write_to_summit_bed(
            ofhd_summits,
            name_prefix="%s_peak_".encode(),
            name=options.name.encode(),
            description=(
                "Summits for %s (Made with MACS v3, " + strftime("%x") + ")"
            ).encode(),
            score_column=score_column,
            trackline=options.trackline,
        )
    ...
    # 4.2 broad peaks in bed12 or gappedPeak
        ...
        info(
            "#4 Write broad peak in broadPeak format file... %s"
            % (options.peakBroadPeak)
        )
        ofhd_bed = open(options.peakBroadPeak, "w")
        peakdetect.peaks.write_to_broadPeak(
            ofhd_bed,
            name_prefix=b"%s_peak_",
            name=options.name.encode(),
            description=options.name.encode(),
            score_column=score_column,
            trackline=options.trackline,
        )
        ofhd_bed.close()
        info(
            "#4 Write broad peak in bed12/gappedPeak format file... %s"
            % (options.peakGappedPeak)
        )
        ofhd_bed = open(options.peakGappedPeak, "w")
        peakdetect.peaks.write_to_gappedPeak(
            ofhd_bed,
            name_prefix=b"%s_peak_",
            name=options.name.encode(),
            description=options.name.encode(),
...
def load_frag_files_options(options):
    ...
    if options.cfile:
        ...
        control = None
    options.info(
        "#1 mean fragment size is determined as %.1f bp from treatment" % options.tsize
    )
    # options.info("#1 fragment size variance is determined as %d bp from treatment" % tp.variance)
    if control is not None:
        options.info(
            "#1 note: mean fragment size in control is %.1f bp -- value ignored"
            % control_d
        )
...
def load_tag_files_options(options):
    ...
    if options.cfile:
        ...
        control = options.parser(
            options.cfile[0], buffer_size=options.buffer_size
        ).build_fwtrack()
        if len(options.cfile) > 1:
            # multiple input
            for cfile in options.cfile[1:]:
