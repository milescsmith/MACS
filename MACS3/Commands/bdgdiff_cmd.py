...
def run(options):
    ...
    twoconditionscore = ScoreTrack.TwoConditionScores(
        t1btrack, c1btrack, t2btrack, c2btrack, depth1, depth2
    )
    twoconditionscore.build()
    twoconditionscore.finalize()
    (cat1, cat2, cat3) = twoconditionscore.call_peaks(
        min_length=options.minlen, max_gap=options.maxgap, cutoff=options.cutoff
    )
    ...
    if options.ofile:
        ...
        ofiles = [
            os.path.join(
                options.outdir, "%s_c%.1f_cond1.bed" % (options.oprefix, options.cutoff)
            ),
            os.path.join(
                options.outdir, "%s_c%.1f_cond2.bed" % (options.oprefix, options.cutoff)
            ),
            os.path.join(
                options.outdir,
    ...
    cat1.write_to_bed(
        ...
        name=b"condition 1",
        description=b"unique regions in condition 1",
        score_column="score",
    )
    nf.close()
    ...
    cat2.write_to_bed(
        ...
        name=b"condition 2",
        description=b"unique regions in condition 2",
        score_column="score",
    )
    nf.close()
    ...
    cat3.write_to_bed(
        ...
        name=b"common",
        description=b"common regions in both conditions",
        score_column="score",
    )
    nf.close()
    info("Done")
