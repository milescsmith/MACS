...
def zwig_write(trackI, subdir, fileprefix, d, log=None, space=10, single=False):
    ...
    if single:
        ...
        wigfhd = open(f, "w")
        wigfhd.write(
            'track type=wiggle_0 name="%s_all" description="Extended tag pileup from MACS version %s for every %d bp"\n'
...
def zbdg_write(trackI, subdir, fileprefix, d, log=None, single=False):
    ...
    if single:
        ...
        bdgfhd = open(f, "w")
        bdgfhd.write(
            'track type=bedGraph name="%s_all" description="Extended tag pileup from MACS version %s"\n'
    ...
    for chrom in sorted(chrs):
        if not single:
            ...
            bdgfhd = open(f, "w")
            bdgfhd.write(
                'track type=bedGraph name="%s_%s" description="Extended tag pileup from MACS version %s"\n'
...
def diag_write(filename, diag_result):
    ...
    ofhd_diag.write(
        "FC range\t# of Peaks\tcover by sampling %s\n" % ("%\t".join(map(str, s)) + "%")
    )
    format = "%s\t%d" + "\t%.2f" * l + "\n"
